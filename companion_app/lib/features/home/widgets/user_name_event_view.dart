...
class UserNameEventView extends StatefulWidget {
  const UserNameEventView({
    super.key,
  ...
  @override
  State<UserNameEventView> createState() => _UserNameEventViewState();
}

class _UserNameEventViewState extends State<UserNameEventView> {
  ...
  Widget build(BuildContext context) {
    ...
    return BottomActionGroup(
      groupKey: const ValueKey('actions-user-name-event'),
      children: [
        TextField(
        ...
        FilledButton(
          key: const ValueKey('user-name-save-button'),
          onPressed: canSave ? () => widget.onSave(proposedName) : null,
          child: const Text('Lagre navn'),
        ),
        const SizedBox(height: 8),
        OutlinedButton(
          key: const ValueKey('user-name-skip-button'),
