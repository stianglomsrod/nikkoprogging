...
void main() {
  ...
  group('Database test isolation and cleanup', () {
    ...
    test(
      'companion event state repository isolation',
      () async {
        final db1 = AppDatabase(NativeDatabase.memory());
    ...
    test(
      'companion identity repository isolation',
      () async {
        final db1 = AppDatabase(NativeDatabase.memory());
    ...
    test(
      'successive read/write cycles maintain isolation',
      () async {
        final db1 = AppDatabase(NativeDatabase.memory());
        final repo1 = DriftCompanionEventStateRepository(db1);
    ...
    test(
      'database closes without error',
      () async {
        final db1 = AppDatabase(NativeDatabase.memory());
        final repo1 = DriftHistoryRepository(db1);
        await repo1.initialize();

        expect(() async => await db1.close(), returnsNormally,
  ...
  group('Test cleanup pattern validation', () {
    test(
      'three-way repository isolation after setup',
      () async {
